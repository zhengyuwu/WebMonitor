...
class crawlLinks:
    ...
    def startCrawl(self, sleep=3600):
        ...
        while True:
            ...
            for baseUrl in baseWebs.keys():
                ...
                t = time.time()
                self.linkSpider(baseUrl, self.urls, baseUrl)
    ...
    def linkSpider(self, url, urls, baseUrl):
        print u'爬虫线程：开始单线程抓取页面[%s]上的链接，初始页面是[%s]' % (url, baseUrl)
        ...
        for newUrl in newUrlSet:
            ...
            urls[newUrl] = 0
            # 对新抓取到的链接进行递归调用
            self.linkSpider(newUrl, urls, baseUrl)
